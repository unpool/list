    ...
    <div class="app">
        <div class="wrapper">
            <div class="chat-area">
                <div class="chat-area-main">
                    <div class="chat-msg">
                        <div class="chat-msg-content">
                            @if(count($massagesData))
                                @foreach($massagesData as $massageData)

                                    @if(!setBoolean($massageData['is_file']))
                                        <div class="chat-msg-text">{{setBoolean($massageData['is_file'])}} {{ $massageData['body'] }}</div>
                                    @else
                                        <div class="chat-msg-text">@include('layouts.admin.partial.showFile', ['file' => $massageData['fileMassage']])</div>
                                    @endif
                                @endforeach
                            @endif
                        </div>
                ...
                <form action="{{ route('admin.channel.send_new_massage', $channel['id']) }}"method="post">
                    @csrf
                    <div class="chat-area-footer">
                        <button type="button" class="btn" data-toggle="modal" data-target="#exampleModal" >فایل</button>
                        <textarea name="body"></textarea>
                        <button type="submit" class="btn btn-info send">ارسال</button>
                    </div>
                </form>
            ...
            @if($channel['type'] == "private")
                ...
                @if(count($usersChannel) != 0)
                    @foreach($usersChannel as $userChannel)
                        <div class="msg">
                            <img class="msg-profile" src="{{ url('User-Profile.png') }}" alt="" />
                            <div class="msg-detail">
                                <div class="msg-username float-right">{{ $userChannel->fullName }}</div>
                                <a href="{{ route('admin.channel.remove_user_as_channel',['channel' => $channel['id'], 'user'=> $userChannel["id"]]) }}" class="btn btn-danger btn-sm mr-5 float-left">حذف</a>
                            </div>
                        </div>
                    @endforeach
                @endif

                <button type="button" data-toggle="modal" data-target="#exampleModal" class="add"></button>
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{ route('admin.channel.add_user_to_channel', $channel['id']) }}" method="post">
            ...
            <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="exampleFormControlSelect1">کاربران</label>
                        <select class="js-example-responsive" name="users[]" multiple="multiple" style="width: 75%" id="exampleFormControlSelect1">
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{ route('admin.channel.send_new_file', $channel['id']) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class=" modal-body">
                    ...
                    <div class="upload_file container">

                                <div class="card-title">فایل خود را انتخاب کنید</div>
                                <div class="file-upload">
                                    <input class="file-input" id="upload" name="file" onchange="chngeUploadFile()" type="file"  accept="image/jpeg,image/gif,image/png,.doc,.mp4" >
                                    ...
                                    <div class="card-subtitle" id="showNameFile">پرونده خود را در اینجا بکشید و رها کنید</div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
...
@section('css')
    <link rel="stylesheet" type="text/css" href="{{ url('channel/style.css') }}"/>
...
@section('js')
    ...
    <script src="{{ asset('channel/script.js') }}"></script>
    <script>
        function chngeUploadFile(){
            ...
            if (fullPath) {
                ...
                document.getElementById("showNameFile").innerHTML = filename;
            }
        }
